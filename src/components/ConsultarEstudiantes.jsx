...
function ConsultarEstudiantes() {
  ...
  useEffect(() => {
    ...
    const fetchEventOptions = async () => {
      const actividadCollection = collection(db, 'usuario');
      ...
      try {
        ...
        const options = actividadSnapshot.docs.map((doc) => ({
          id: doc.data().carnet,
          name: doc.data().carnet, // Asumiendo que tienes un campo nombreEvento en tus documentos
        }));
  ...
  const handleNavigate = () => {
    if (selectedActividad!==""){
        navigate('/gestionarEstudiante', { state: { carnet: selectedActividad } })
    }
  };
